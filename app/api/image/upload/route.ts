...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('TENCENT_COS_BUCKET:', process.env.TENCENT_COS_BUCKET || '[NOT SET]');
    console.log('TENCENT_COS_REGION:', process.env.TENCENT_COS_REGION || '[NOT SET]');
    ...
    // Get environment variables (图片去水印配置)
    const secretId = process.env.TENCENT_SECRET_ID;
    const secretKey = process.env.TENCENT_SECRET_KEY;
    const bucket = process.env.TENCENT_COS_BUCKET_IMAGE;
    const region = process.env.TENCENT_COS_REGION_IMAGE;
    ...
    const fileName = `uploads/${Date.now()}-${Math.random().toString(36).substring(7)}.${file.name.split('.').pop()}`;
